...
var GameLayer = cc.Layer.extend({
	...
	init:function() {
		...
		this._hishScoreLabel = this.addLabel("10000", kScoreX, size.height - kScoreY * 3, TRUE, kZindexRobin,
		cc.color.RED, kFontSizeScore );
		this._scoreLabel.setAnchorPoint(0,1);
		this._hishScoreLabel.setAnchorPoint(0,1);
	...
	setScoreLabels:function() {
		...
		this._hishScoreLabel.string = this._highScore.toString();
	},
					...
					if(ArrayTubes[i].state == kTubeStateActive) {
						if(cc.rectIntersectsRect(ArrayTubes[i].getBoundingBox(), RobinCollBox) == TRUE) {
							...
							if(ArrayTubes[i].scored == FALSE) {
								if(ArrayTubes[i].getBoundingBox().x + 
									ArrayTubes[i].getBoundingBox().width < 
